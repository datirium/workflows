...
doc: |
  ...
  *Kartashov AV, Barski A. BioWardrobe: an integrated platform for analysis of epigenomics and transcriptomics data. Genome Biol. 2015;16(1):158. Published 2015 Aug 7. [doi:10.1186/s13059-015-0720-3](https://www.ncbi.nlm.nih.gov/pubmed/26248465)*
  ...
  Reads are aligned to the reference genome with Bowtie. Results are saved as coordinate sorted BAM alignment and index BAI files. Optionally, PCR duplicates can be removed. To obtain coverage in bigWig format, average fragment length is calculated by MACS2, and individual reads are extended to this length in the 3’ direction. Areas of enrichment identified by MACS2 are saved in ENCODE narrow peak or broad peak formats. Called peaks together with the nearest genes are saved in TSV format. In addition to basic statistics (total/mapped/multi-mapped/unmapped/duplicate reads), pipeline generates several quality control measures. Base frequency plots are used to estimate adapter contamination, a frequent occurrence in low-input ChIP-Seq experiments. Expected distinct reads count from Preseq can be used to estimate read redundancy for a given sequencing depth. Average tag density profiles can be used to estimate ChIP enrichment for promoter proximal histone modifications. Use of different parameters for different antibodies (calling broad or narrow peaks) is possible. Additionally, users can elect to use BAM file from another experiment as control for MACS2 peak calling.
  ...
  | ----------------------- | ---------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------: | ------- | ----------------------------- |
  | **fastq_file**          | FASTQ file                                     | Single-read sequencing data in FASTQ format (fastq, fq, bzip2, gzip, zip)                                                                                      | <img src="https://img.icons8.com/office/16/000000/checkmark.png"> |         |                               |
  | **indices_folder**      | Genome indices                                 | Directory with the genome indices generated by Bowtie                                                                                                          | <img src="https://img.icons8.com/office/16/000000/checkmark.png"> |         | genome_indices/bowtie_indices |
  | **annotation_file**     | Genome annotation file                         | Genome annotation file in TSV format                                                                                                                           | <img src="https://img.icons8.com/office/16/000000/checkmark.png"> |         | genome_indices/annotation     |
  | **genome_size**         | Effective genome size                          | The length of the mappable genome (hs, mm, ce, dm or number, for example 2.7e9)                                                                                | <img src="https://img.icons8.com/office/16/000000/checkmark.png"> |         | genome_indices/genome_size    |
  | **chrom_length**        | Chromosome lengths file                        | Chromosome lengths file in TSV format                                                                                                                          | <img src="https://img.icons8.com/office/16/000000/checkmark.png"> |         | genome_indices/chrom_length   |
  | **broad_peak**          | Call broad peaks                               | Make MACS2 call broad peaks by linking nearby highly enriched regions                                                                                          | <img src="https://img.icons8.com/office/16/000000/checkmark.png"> |         |                               |
  | **control_file**        | Control ChIP-Seq single-read experiment        | Indexed BAM file from the ChIP-Seq single-read experiment to be used as a control for MACS2 peak calling                                                       |                                                                   | Null    | control_file/bambai_pair      |
  | **exp_fragment_size**   | Expected fragment size                         | Expected fragment size for read extenstion towards 3' end if *force_fragment_size* was set to True or if calculated by MACS2 fragment size was less that 80 bp |                                                                   | 150     |                               |
  | **force_fragment_size** | Force peak calling with expected fragment size | Make MACS2 don't build the shifting model and use expected fragment size for read extenstion towards 3' end                                                    |                                                                   | False   |                               |
  | **clip_3p_end**         | Clip from 3' end                               | Number of base pairs to clip from 3' end                                                                                                                       |                                                                   | 0       |                               |
  | **clip_5p_end**         | Clip from 5' end                               | Number of base pairs to clip from 5' end                                                                                                                       |                                                                   | 0       |                               |
  | **remove_duplicates**   | Remove PCR duplicates                          | Remove PCR duplicates from sorted BAM file                                                                                                                     |                                                                   | False   |                               |
  ...
  | ----------------------- | ---------------------------------- | ---------------------------------------------------------------------------------- | :---------------------------------------------------------------: | ------------------------------------------------------------------ |
  | **fastx_statistics**    | FASTQ quality statistics           | FASTQ quality statistics in TSV format                                             | <img src="https://img.icons8.com/office/16/000000/checkmark.png"> | *Base Frequency* and *Quality Control* plots in *QC Plots* tab     |
  | **bambai_pair**         | Aligned reads                      | Coordinate sorted BAM alignment and index BAI files                                | <img src="https://img.icons8.com/office/16/000000/checkmark.png"> | *Nucleotide Sequence Alignments* track in *IGV Genome Browser* tab |
  | **bigwig**              | Genome coverage                    | Genome coverage in bigWig format                                                   | <img src="https://img.icons8.com/office/16/000000/checkmark.png"> | *Genome Coverage* track in *IGV Genome Browser* tab                |
  | **iaintersect_result**  | Gene annotated peaks               | MACS2 peak file annotated with nearby genes                                        | <img src="https://img.icons8.com/office/16/000000/checkmark.png"> | *Peak Coordinates* table in *Peak Calling* tab                     |
  | **atdp_result**         | Average Tag Density Plot           | Average Tag Density Plot file in TSV format                                        | <img src="https://img.icons8.com/office/16/000000/checkmark.png"> | *Average Tag Density Plot* in *QC Plots* tab                       |
  | **macs2_called_peaks**  | Called peaks                       | Called peaks file with 1-based coordinates in XLS format                           | <img src="https://img.icons8.com/office/16/000000/checkmark.png"> |                                                                    |
  | **macs2_narrow_peaks**  | Narrow peaks                       | Called peaks file in ENCODE narrow peak format                                     |                                                                   | *Narrow peaks* track in *IGV Genome Browser* tab                   |
  | **macs2_broad_peaks**   | Broad peaks                        | Called peaks file in ENCODE broad peak format                                      |                                                                   | *Broad peaks* track in *IGV Genome Browser* tab                    |
  | **preseq_estimates**    | Expected Distinct Reads Count Plot | Expected distinct reads count file from Preseq in TSV format                       |                                                                   | *Expected Distinct Reads Count Plot* in *QC Plots* tab             |
  | **workflow_statistics** | Workflow execution statistics      | Overall workflow execution statistics from bowtie_aligner and samtools_rmdup steps | <img src="https://img.icons8.com/office/16/000000/checkmark.png"> | *Overview* tab and experiment's preview                            |
  | **bowtie_log**          | Read alignment log                 | Read alignment log file from Bowtie                                                | <img src="https://img.icons8.com/office/16/000000/checkmark.png"> |                                                                    |
