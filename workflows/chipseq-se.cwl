...
doc: |
  ...
  *Kartashov AV, Barski A. BioWardrobe: an integrated platform for analysis of epigenomics and transcriptomics data. Genome Biol. 2015;16(1):158. Published 2015 Aug 7. [doi:10.1186/s13059-015-0720-3](https://www.ncbi.nlm.nih.gov/pubmed/26248465)*
  ...
  | ----------------------- | ---------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------: | ------- | ----------------------------- |
  | **fastq_file**          | FASTQ file                                     | Single-read sequencing data in FASTQ format (fastq, fq, bzip2, gzip, zip)                                                                                      |    +     |         |                               |
  | **indices_folder**      | Genome indices                                 | Directory with the genome indices generated by Bowtie                                                                                                          |    +     |         | genome_indices/bowtie_indices |
  | **annotation_file**     | Genome annotation file                         | Genome annotation file in TSV format                                                                                                                           |    +     |         | genome_indices/annotation     |
  | **genome_size**         | Effective genome size                          | The length of the mappable genome (hs, mm, ce, dm or number, for example 2.7e9)                                                                                |    +     |         | genome_indices/genome_size    |
  | **chrom_length**        | Chromosome lengths file                        | Chromosome lengths file in TSV format                                                                                                                          |    +     |         | genome_indices/chrom_length   |
  | **broad_peak**          | Call broad peaks                               | Make MACS2 call broad peaks by linking nearby highly enriched regions                                                                                          |    +     |         |                               |
  ...
  | ----------------------- | ---------------------------------- | ---------------------------------------------------------------------------------- | :------: | ------------------------------------------------------------------ |
  | **fastx_statistics**    | FASTQ quality statistics           | FASTQ quality statistics in TSV format                                             |    +     | *Base Frequency* and *Quality Control* plots in *QC Plots* tab     |
  | **bambai_pair**         | Aligned reads                      | Coordinate sorted BAM alignment and index BAI files                                |    +     | *Nucleotide Sequence Alignments* track in *IGV Genome Browser* tab |
  | **bigwig**              | Genome coverage                    | Genome coverage in bigWig format                                                   |    +     | *Genome Coverage* track in *IGV Genome Browser* tab                |
  | **iaintersect_result**  | Gene annotated peaks               | MACS2 peak file annotated with nearby genes                                        |    +     | *Peak Coordinates* table in *Peak Calling* tab                     |
  | **atdp_result**         | Average Tag Density Plot           | Average Tag Density Plot file in TSV format                                        |    +     | *Average Tag Density Plot* in *QC Plots* tab                       |
  | **macs2_called_peaks**  | Called peaks                       | Called peaks file with 1-based coordinates in XLS format                           |    +     |                                                                    |
  ...
  | **workflow_statistics** | Workflow execution statistics      | Overall workflow execution statistics from bowtie_aligner and samtools_rmdup steps |    +     | *Overview* tab and experiment's preview                            |
  | **bowtie_log**          | Read alignment log                 | Read alignment log file from Bowtie                                                |    +     |                                                                    |