...
inputs:
  ...
  indices_folder_for_spikein:
    ...
    sd:localLabel: true
    doc: preprocessed genome index of spike-in organism for secondary alignment (of unaligned reads from primary alignment) and spike-in normalization, default should be E. coli K-12
  annotation_file:
    type: File
  ...
  fragment_length_filter:
    ...
    sd:localLabel: true
    doc: 'Fragment Length Filter options: 1) default_below_1000 retains fragments <1000 bp, 2) histones_130_to_300 retains fragments between 130-300 bp, and 3) TF_below_130 retains fragments <130 bp.'
  numeric_threshold:
    type: float?
    default: 0.01
    sd:layout:
...
steps:
  ...
  convert_bed_to_xls_relaxed:
    ...
    in:
      ...
      script:
        default: |
          cat $0 | awk -F'\t' '{if($3>$2){printf("%s\t%.0f\t%.0f\t%.0f\t%.0f\t%.0f\t0\t0\t0\t%s\n",$1,$2,$3,$3-$2+1,$2+(($3-$2)/2),$4,"peak_"NR)}}' > `basename $0`".bed"
    out:
    - output_file
    doc: |
  ...
  convert_bed_to_xls_for_iaintersect:
    ...
    in:
      ...
      script:
        default: |
          cat $0 | awk -F'\t' 'BEGIN {print "chr\tstart\tend\tlength\tabs_summit\tpileup\t-log10(pvalue)\tfold_enrichment\t-log10(qvalue)\tname"} {if($3>$2){printf("%s\t%.0f\t%.0f\t%.0f\t%.0f\t%.0f\t0\t0\t0\t%s\n",$1,$2,$3,$3-$2+1,$2+(($3-$2)/2),$4,"peak_"NR)}}' > `basename $0`"headers.bed"
    out:
    - output_file
    doc: |
