...
inputs:
  ...
  vdj_fastq_file_r1:
    type: File
    doc: |
      V(D)J FASTQ read 1 file.
      It will be staged into workdir
      as vdj_S1_L001_R1_001.fastq.

  vdj_fastq_file_r2:
    type: File
    doc: |
      V(D)J FASTQ read 2 file.
      It will be staged into workdir
      as vdj_S1_L001_R2_001.fastq.

  vdj_fastq_file_i1:
    type: File?
    doc: |
      V(D)J FASTQ index i7 file.
      It will be staged into workdir
      as vdj_S1_L001_I1_001.fastq.

  vdj_fastq_file_i2:
    type: File?
    doc: |
      V(D)J FASTQ index i5 file.
      It will be staged into workdir
      as vdj_S1_L001_I2_001.fastq.

  rna_indices_folder:
    type: Directory
    doc: |
      Path of folder containing 10x-
      compatible transcriptome reference.
      Should be generated by "cellranger
      mkref" command.

  vdj_indices_folder:
    type: Directory
    doc: |
      Path of folder containing Cell Ranger
      V(D)J-compatible reference. Should be
      generated by "cellranger mkvdjref"
      command.
  ...
  vdj_chain_type:
    ...
    doc: |
      ...
      auto-detect the chain type. Auto-detection
      does not work for TRG/D (gamma-delta)
      chains. Note that gamma-delta analysis is
      enabled but the algorithm has not been
      tested extensively.
  ...
  threads:
    ...
    doc: |
      ...
      at one time. Default: all available.
  ...
  virt_memory_limit:
    ...
    inputBinding:
      ...
      prefix: "--localvmem"
    doc: |
      Set max virtual address space in GB
      for the pipeline. Default: all available.
...
outputs:
  ...
  filtered_feature_bc_matrix_folder:
    ...
    doc: |
      ...
      barcodes in MEX format. Each element
      of the matrix is the number of UMIs
      associated with a feature (row) and
      a barcode (column).
  ...
  filtered_feature_bc_matrix_h5:
    ...
    doc: |
      ...
      in HDF5 format. Each element of the
      matrix is the number of UMIs
      associated with a feature (row) and a
      barcode (column).
  ...
  raw_feature_bc_matrices_folder:
    ...
    doc: |
      Folder with unfiltered feature-barcode
      matrices containing all barcodes in MEX
      format. Each element of the matrix is
      the number of UMIs associated with a
      feature (row) and a barcode (column).
  ...
  raw_feature_bc_matrices_h5:
    ...
    doc: |
      ...
      Each element of the matrix is the number
      of UMIs associated with a feature (row)
      and a barcode (column).
  ...
  secondary_analysis_report_folder:
    ...
    doc: |
      Folder with secondary analysis of
      RNA data including dimensionality
      reduction, cell clustering, and
  ...
  all_contig_reads_bam_bai:
    ...
    doc: |
      ...
      This file demonstrates how the reads and UMIs
      support the assembled contigs within a cell
      barcode. Reads are not aligned across cell
      barcode boundaries. Please note that this BAM
      excludes reads whose barcodes don't match the
      whitelist, so it is not suitable as an archive
      of every single input read. This file includes
      reads from all cells barcodes identified by
      V(D)J algorithm including those ones that will
      be later discarded as non-viable cells by V(D)J
      algorithm and those barcodes that will be later
      removed after overlapping with cells called by
      RNA algorithm.
  ...
  all_contig_sequences_fasta:
    ...
    doc: |
      ...
      productive and non-productive contigs with high
      and low confidence assembled for all identified
      cells barcodes including those ones that will be
      later discarded as non-viable cells by V(D)J
      algorithm or after overlapping with cells called
      by RNA algorithm.
  ...
  all_contig_annotations_bed:
    ...
    doc: |
      BED file with high-level and detailed annotations
      of ALL assembled contigs (from cell and background
      barcodes). Used for further investigation into why
      some contigs were filtered out. This file includes
      both productive and non-productive contigs with high
      and low confidence assembled for all identified cells
      barcodes including those ones that will be later
      discarded as non-viable cells by V(D)J algorithm or
      after overlapping with cells called by RNA algorithm.
  ...
  all_contig_annotations_csv:
    ...
    doc: |
      CSV file with high-level and detailed annotations
      of ALL assembled contigs (from cell and background
      barcodes). Used for further investigation into why
      some contigs were filtered out. This file includes
      both productive and non-productive contigs with high
  ...
  clonotypes_csv:
    ...
    doc: |
      CSV file with high-level descriptions of each
      clonotype. During the clonotype grouping stage,
      cell barcodes are placed in groups called
      clonotypes. Only viable cells identified by both
      V(D)J and RNA algorithms are used. Each clonotype
      consists of all descendants of a single, fully
      rearranged common ancestor, as approximated
      computationally. During this process, some cell
      barcodes are flagged as likely artifacts and
      filtered out, meaning that they are no longer
      called as cells. However, as clonotype grouping
      stage is hapenning before forming the final version
      of files in the per_sample_outs folder, the reported
      cells number won't be affected.
  ...
  germline_contigs_bam_bai:
    ...
    doc: |
      ...
      consensus, the reference sequence is the annotated
      germline segments concatenated together. This file
      shows how both the per-cell contigs and the clonotype
      consensus contig relate to the germline reference.
      Useful for revealing polymorphisms, somatic mutations,
      and recombination-induced differences such as
  ...
  germline_sequences_fasta:
    ...
    doc: |
      ...
      These serve as an approximate reference for
  ...
  consensus_contigs_bam_bai:
    ...
    doc: |
      ...
      is a clonotype consensus sequence, and each
      record is an alignment of a single cell's contig
      against this consensus. This file shows, for a
      clonotype consensus sequences, how the constituent
  ...
  consensus_annotations_csv:
    ...
    doc: |
      CSV file with high-level and detailed annotations
      of each clonotype consensus sequence.
  ...
  filtered_contig_annotations_csv:
    ...
    doc: |
      CSV file with high-level annotations of each
      high-confidence contig from cell-associated
      barcodes. This is a subset of
  ...
  filtered_contig_sequences_fasta:
    ...
    doc: |
      FASTA format sequence for only high-confidence
      contigs in cell barcodes.
  ...
  loupe_vdj_browser_track:
    ...
    doc: |
      Loupe V(D)J Browser visualization
      and analysis file
  ...
  filtered_data_folder:
    ...
    doc: |
      Folder containing filtered data, i.e., only
      cell-associated barcodes. Used by cellranger
      aggr to aggregate samples for joint analysis.
