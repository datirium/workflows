...
inputs:
  ...
  r1_length:
    ...
    inputBinding:
      ...
      prefix: "--r1-length"
    doc: |
      Limit the length of the input Read 1 sequence of Gene Expression library
      to the first N bases, where N is a user-supplied value. Note that the length
      includes the 10x Barcode and UMI sequences so do not set this below 26 for
      Single Cell 3′ v2 or Single Cell 5′. This and --r2-length are useful options
      for determining the optimal read length for sequencing.
  ...
  r2_length:
    ...
    inputBinding:
      ...
      prefix: "--r2-length"
    doc: |
      Limit the length of the input R2 sequence to the first N bases, where N is a
      user-supplied value. Trimming occurs before sequencing metrics are computed
      and therefore, limiting R2 read length may affect Q30 scores.
  ...
  expect_cells:
    ...
    doc: |
      ...
      Starting in Cell Ranger 7.0, the expected number of cells can be either auto-estimated
      or specified with --expect-cells. To replicate an old cellranger count analysis, set
      this parameter to 3,000 cells.
  ...
  no_bam:
    ...
    inputBinding:
      ...
      prefix: "--no-bam"
    doc: |
      Set this flag to not generate the BAM file. This will reduce the total computation
      time for the pipestance and the size of the output directory. If unsure, we recommend
      not to use this option. BAM file could be useful for troubleshooting and downstream
      analysis
  ...
  exclude_introns:
    ...
    inputBinding:
      position: 11
      prefix: "--include-introns=false"
    doc: |
      In Cell Ranger v7.0 intronic reads are counted by default for whole transcriptome
      gene expression data, except when --target-panel is used. Therefore, here we provide
      a flag to disable this default behavior.
...
doc: |
  ...
  New in Cell Ranger v7.0: Intronic reads are counted by default for
  whole transcriptome gene expression data. For more details see
...
s:about: |
  Count gene expression (targeted or whole-transcriptome) and/or feature barcode reads
  ...
  OPTIONS:
      ...
      --target-panel <CSV>                          The target panel CSV file declaring the target panel used, if any. Default analysis will exclude intronic mapped reads, which
                                                    is the recommended mode for targeted assay. Use include-introns=true to include intronic mapped reads in analysis
      ...
      --no-bam                                      Set --no-bam to not generate the BAM file. This will reduce the total computation time for the pipestance and the size of the
                                                    output directory. If unsure, we recommend not to use this option. BAM file could be useful for troubleshooting and downstream
                                                    analysis
      ...
      --chemistry <CHEM>                            Assay configuration. NOTE: by default the assay configuration is detected automatically, which is the recommened mode. You
                                                    usually will not need to specify a chemistry. Options are: 'auto' for autodetection, 'threeprime' for Single Cell 3',
                                                    'fiveprime' for  Single Cell 5', 'SC3Pv1' or 'SC3Pv2' or 'SC3Pv3' for Single Cell 3' v1/v2/v3, 'SC3Pv3LT' for Single Cell 3'
                                                    v3 LT, 'SC3Pv3HT' for Single Cell 3' v3 HT, 'SC5P-PE' or 'SC5P-R2' for Single Cell 5', paired-end/R2-only, 'SC-FB' for Single
                                                    Cell Antibody-only 3' v2 or 5' [default: auto]
      ...
      --jobmode <MODE>                              Job manager to use. Valid options: local (default), sge, lsf, slurm or path to a .template file. Search for help on "Cluster
                                                    Mode" at support.10xgenomics.com for more details on configuring the pipeline to use a compute cluster [default: local]
      ...
      --mempercore <NUM>                            Reserve enough threads for each job to ensure enough memory will be available, assuming each core on your cluster has at least
                                                    this much memory available. Only applies to cluster jobmodes
