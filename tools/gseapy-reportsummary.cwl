...
inputs:
  ...
  script:
    ...
    default: |
      ...
      printf "\tThere are $ledge_genes leading edge genes among all marker sets analyzed"
      ...
      printf "%s %s\n" "$sigsets_fdr25" "marker gene sets are significant at FDR <25%" >> reportsummary.txt
      printf "%s %s\n" "$sigsets_pv1" "marker gene sets are significantly enriched at nominal pvalue <1%" >> reportsummary.txt
      printf "%s %s\n" "$sigsets_pv5" "marker gene sets are significantly enriched at nominal pvalue <5%" >> reportsummary.txt
      # phenotype 2
      enriched=$(cut -f2-5 $report | tail -n+2 | awk -F'\t' 'BEGIN{x=0};{if($1<0){x++}}END{print(x)}')
      sigsets_fdr25=$(cut -f2-5 $report | tail -n+2 | awk -F'\t' 'BEGIN{x=0};{if($1<0){if($4<0.25){x++}}}END{print(x)}')
      sigsets_pv1=$(cut -f2-5 $report | tail -n+2 | awk -F'\t' 'BEGIN{x=0};{if($1<0){if($3<0.01){x++}}}END{print(x)}')
